...
void start_round(){
	...
	ball_vel.x = randrange(0, 1*BALL_SPEED/3.0)+2/3.0*BALL_SPEED;
...
void NewGame(){
	...
	web_read_line(Player_1, 12);
	system("cls");
	hideCursor();
	drawBorders();
	gotoxy(WIDTH/2-10,HEIGHT/2);
	printf("Player 2 Name: ");
	web_read_line(Player_2, 12);
	system("cls");
	hideCursor();
	drawBorders();
	gotoxy(WIDTH/2-10,HEIGHT/2);
	printf("Create a Key: ");
	web_read_line(Key, 12);
	score_1 = 0;
	score_2 = 0;
	elapsedTime = 0;
	main_game();
