...
void drawBorders(){
	...
	for(int i=1;i<HEIGHT; i++){gotoxy(0,i);printf("▒");}
	for(int i=1;i<HEIGHT; i++){gotoxy(WIDTH-1,i);printf("▒");}
	gotoxy(0, HEIGHT-1);
...
void drawLayout(){
	gotoxy(0,0);
	for(int i=0;i<WIDTH; i++){printf("▒");}
	for(int i=1;i<HEIGHT; i++){gotoxy(0,i);if(abs(((HEIGHT-1)/2)-i)>(GOAL_SIZE)/2)printf("▒");}
	for(int i=1;i<HEIGHT; i++){gotoxy(WIDTH-1,i);if(abs(((HEIGHT-1)/2)-i)>(GOAL_SIZE)/2)printf("▒");}
	gotoxy(0, HEIGHT-1);
	for(int i=1;i<WIDTH; i++){printf("▒");}
	gotoxy(1,2);
	for(int i=0;i<WIDTH-2; i++){printf("▒");}
	for(int i=3;i<HEIGHT-1; i++){gotoxy(WIDTH/2,i);if(i%2)printf("│");}
}
